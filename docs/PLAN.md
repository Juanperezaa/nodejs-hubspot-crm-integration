...
**Merge strategy: rebase, preserving every atomic commit. Never squash.**
...
Rebase merging keeps `main` linear while preserving every commit, so `git log`
reads as a continuous account of why each decision was taken, not merely what
shipped. The repository is configured to allow rebase merges only.

> Pull request 1 carries a merge commit: it was merged before this setting was
> applied. Every pull request from 2 onwards is rebased. The inconsistency is
> recorded here rather than rewritten away, because force-pushing `main` to
> tidy history is a worse habit than one irregular commit.
